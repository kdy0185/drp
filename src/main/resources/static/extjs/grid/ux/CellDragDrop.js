Ext.define('Ext.view.CellDropZone', {
    ...
    getTargetFromEvent: function (e) {
        ...
        if (cell) {
            ...
            var gridPanel = self.view.up('treepanel') || self.view.up('grid'); 
            var hiddenIndex = 0;
            var showIndex = 0;
            var realColumnIndex;
            Ext.each(gridPanel.columns, function(column, index) {
                   if(column.isHidden()){
                       hiddenIndex++;
                   } else {
                       showIndex++;
                   ...
                   if(columnIndex == showIndex){
                       realColumnIndex = showIndex + hiddenIndex; 
                       return;
                   }
            });
            ...
            if (row && Ext.isDefined(columnIndex)) {
                return {
                    ...
                    columnName: gridPanel.columns[realColumnIndex].dataIndex
                };
            }
        }
    },
    ...
    onNodeEnter: function (target, dd, e, dragData) {
        ...
        // If you set the enableSelfDrop entry to false, it will return if the grid's id is the same.
        if(!self.enableSelfDrop){
            var dropGridPanel = self.view.up('treepanel') || self.view.up('grid');
            var dragGridPanel = dd.view.up('treepanel') || dd.view.up('grid')
            var dropGridId = dropGridPanel.getId();
            var dragGridId = dragGridPanel.getId();
            if(dropGridId == dragGridId){
                return;
            }
        }

        isBeforeNodeEnter = self.fireViewEvent('beforeNodeEnter', target, dragData);
        if (enterHandlers.wait) {
            return;
        }
        if (isBeforeNodeEnter !== false) {
            if (!enterHandled) {
            	enterHandlers.processEnter();
            }
        }

        if (!isBeforeNodeEnter){
            return;
        }

        // Check whether the data type of the column being dropped on accepts the
        // dragged field type. If so, set dropOK flag, and highlight the target node.
        if (self.enforceType && destType !== sourceType) {
        ...
        if (self.dropCls) {
            Ext.fly(target.node).addCls(self.dropCls);
        } else {
            Ext.fly(target.node).applyStyles({
                backgroundColor: self.dropBackgroundColor
            });
        }
    },
    ...
    onNodeDrop: function(targetNode, dragZone, e, data) {
        var me = this,
            ...
            dropHandlers = {
                ...
                processDrop: function () {
                    ...
                    if(me.changeDropData){
                        if(targetNode.record){
                        	targetNode.record.set(targetNode.columnName, data.record.get(data.columnName));
                        }
                    }

                    if (me.applyEmptyText) {
                        data.record.set(data.columnName, me.emptyText);
                    }

                    dropHandled = true;
                    me.fireViewEvent('drop', targetNode, targetNode.record, data, data.record);
                },
        ...
        if (me.dropOK) {
            performOperation = me.fireViewEvent('beforedrop', targetNode, targetNode.record, data, data.record);
...
Ext.define('Ext.ux.CellDragDrop', {
    ...
    /**
     * @cfg {Boolean} enableDrop
     * Set to `false` to not Drop to yourself.
     */
    enableSelfDrop : true,
    
    /**
     * @cfg {Boolean} enableDrop
     * Set to `false` to not allow data changes.
     */
    changeDropData: true,
    ...
    onViewRender: function (view) {
        ...
        if (me.enableDrag) {
            ...
            me.dragZone = new Ext.view.DragZone({
                ...
                getDragData: function (e) {
                    ...
                    if (item) {
                        var cellDragTxt = record.get(me.cellDragRecord) || clickedEl.textContent || clickedEl.innerText;
                        dragEl = document.createElement('div');
                        dragEl.className = 'x-form-text';
                        dragEl.appendChild(document.createTextNode(cellDragTxt));
        ...
        if (me.enableDrop) {
            me.dropZone = new Ext.view.CellDropZone({
                view: view,
                ddGroup: me.dropGroup || me.ddGroup,
                containerScroll: true,
                cellDragRecord : me.cellDragRecord,
                changeDropData : me.changeDropData,
                enableDrag : me.enableDrag,
                enableDrop : me.enableDrop,
                enableSelfDrop : me.enableSelfDrop,
                applyEmptyText : me.applyEmptyText,
                emptyText : me.emptyText,
                dropBackgroundColor : me.dropBackgroundColor
            });
        }
    }
});
