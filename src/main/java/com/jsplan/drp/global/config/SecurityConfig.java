...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
            ...
            .and()
            .sessionManagement()
            .sessionAuthenticationStrategy(sessionAuthenticationStrategy()) // 동시 세션 처리 전략 설정
            .and()
            .httpBasic().authenticationEntryPoint(authenticationEntryPoint()) // 인증 Entry Point 지정
            .and()
            .addFilterAt(concurrentSessionFilter(),
                ConcurrentSessionFilter.class) // 사용자 정의 filter 추가 : 3. concurrentSessionFilter
            ...
            .addFilterAt(usernamePasswordAuthenticationFilter(),
                UsernamePasswordAuthenticationFilter.class) // 사용자 정의 filter 추가 : 10. usernamePasswordAuthenticationFilter
            .addFilterAt(rememberMeAuthenticationFilter(),
                RememberMeAuthenticationFilter.class) // 사용자 정의 filter 추가 : 14. rememberMeAuthenticationFilter
            .addFilterBefore(exceptionTranslationFilter(),
                ExceptionTranslationFilter.class) // 사용자 정의 filter 추가 : 17. exceptionTranslationFilter
            .addFilterAfter(ajaxSessionTimeoutFilter(),
                ExceptionTranslationFilter.class) // 사용자 정의 filter 추가 : 17. ajaxSessionTimeoutFilter
            .addFilterBefore(filterSecurityInterceptor(),
                FilterSecurityInterceptor.class) // 사용자 정의 filter 추가 : 18. filterSecurityInterceptor
        ;
        ...
        if ("real".equals(profiles)) {
            http.requiresChannel().antMatchers("/**")
                .requiresSecure(); // 운영 환경에서 SSL 적용 (https + 포트 전환)
        }
    }
     ...
     * (ConcurrentSessionControlAuthenticationStrategy 객체)
     */
    @Bean
    public ConcurrentSessionControlAuthenticationStrategy concurentSessionStrategy() {
        ...
        concurentSessionStrategy.setExceptionIfMaximumExceeded(
            false); // false : 먼저 로그인 한 사용자 차단 / true : 나중에 로그인 한 사용자 차단
    ...
    public UserAuthFailureHandler userAuthFailureHandler() {
        ...
        userAuthFailureHandler.setDefaultFailureUrl(
            "/main/login/login.do?failure=true"); // 로그인 실패 후 이동할 기본 URL
     ...
     * (ReloadableFilterInvocationSecurityMetadataSource 객체)
     * @throws Exception throws Exception
     */
    @Bean
